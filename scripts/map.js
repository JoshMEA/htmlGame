...
const $scene = $gameScreen.querySelector('.scene-screen');
...
// DEFINING OBJECTS
let objectsArray = 
[   new Object(1, 396, 176, 178, 80, '', 'table'),
    new Object(1, 687, 181, 104, 90, '', 'chair'),
    new Object(1, 370, 30, 285, 100, '', 'lounge')
];
...
let Scene = function(sceneNum, name, enemies) {
    this.name = name;
    this.sceneNum = sceneNum;
    this.imgSrc = `images/scene${sceneNum}.jpg`;
    this.objects = objectsArray.filter(obj => obj.sceneNum === sceneNum);
    this.enemies = enemies;
    // scenes.push(this);
}

let scenes = 
[   new Scene(1, 'Opening Scene', undefined),
    new Scene(2, 'Opening Scene', undefined)
];
...
scenes[currentScene].objects.forEach(obj => {
    ...
    $objects[index].style.top = `${scenes[currentScene].objects[index].props.startY}px`; // sets start y coords
    $objects[index].style.left = `${scenes[currentScene].objects[index].props.startX}px`;// sets start x coords
